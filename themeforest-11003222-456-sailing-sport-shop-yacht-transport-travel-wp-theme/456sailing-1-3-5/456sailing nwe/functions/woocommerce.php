...
if ( ! function_exists( 'woocommerce_get_product_thumbnail' ) ) {
	...
	function woocommerce_get_product_thumbnail( $size = 'shop_catalog', $placeholder_width = 0, $placeholder_height = 0  ) {
		...
		$output .= '<div class="loop-shop-btn">';

		if(!$catalog_type){
		
			$output .= apply_filters( 'woocommerce_loop_add_to_cart_link',
				sprintf( '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" class="lpd-product-btn %s product_type_%s">%s</a>',
			...
			$output .= '<a href="'.get_permalink().'" rel="nofollow" class="lpd-product-btn">';
			$output .= esc_html__('view more', GETTEXT_DOMAIN);
			$output .= '</a>';
	...
	ob_start();?>
		...
		<div class='lpd-shopping-cart-wrap'>
			...
			<div class="cart-dropdown hidden-xs hidden-sm">
				...
				<div class="lpd-shopping-cart-title"><?php esc_html_e( 'Shopping Cart', GETTEXT_DOMAIN ); ?></div>
				<div class="lpd-shopping-cart-list clearfix">
		...
		if (is_plugin_active('woocommerce/woocommerce.php')) {
			...
			if ($shop_custom_css_enable!="") {
				if (is_shop()||is_tax('product_cat')||is_tax('product_tag')) {
					...
					Custom Shop Styles
					-------------------------------------------------------------------*/
						<?php echo esc_html($shop_custom_css);?>
					</style>
