...
function h1($atts, $content=null){
	...
	$Type = '';
	if($type){ $Type = 'class="'.esc_attr($type).'"';};
...
function abbr($atts, $content=null){
	...
	return '<abbr title="'.esc_attr($title).'">'.do_shortcode($content).'</abbr>';
}
...
function dropcap($atts, $content=null){
	...
	return '<span class="dropcap ' . esc_attr($type) . '">'.do_shortcode($content).'</span>';
}
...
function dropcap1($atts, $content=null){
	...
	return '<span class="dropcap1 ' . esc_attr($type) . '">'.do_shortcode($content).'</span>';
}
...
function dropcap2($atts, $content=null){
	...
	return '<span class="dropcap2 ' . esc_attr($type) . '">'.do_shortcode($content).'</span>';
}
...
function label( $atts, $content = null ) {
    ...
    $out .= '<span class="label '.esc_attr($type_).'">'.do_shortcode($content).'</span>';
    return $out;
}
...
function hyperlink($atts, $content=null){
	...
	return '<a href="'.esc_url($href).'" target="'.esc_attr($target).'">'.do_shortcode($content).'</a>';
}
...
function tooltip($atts, $content=null){
	...
	return '<a href="#" class="lpd-tooltip" data-toggle="tooltip" data-placement="'.esc_attr($placement).'" title="'.esc_attr($title).'">'.do_shortcode($content).'</a>';
}
...
function table( $atts, $content = null ) {
    extract(shortcode_atts(array('type' => ''), $atts));
	$out = '';
    $out .= '<table class="table '.esc_attr($type).'">'.do_shortcode(pre_table($content)).'</table>';
    return $out;
}
...
function tr( $atts, $content = null ) {
    extract(shortcode_atts(array('type' => ''), $atts));
	$out = '';
    $out .= '<tr class="'.esc_attr($type).'">'.do_shortcode(pre_table($content)).'</tr>';
    return $out;
}
add_shortcode('tr', 'tr');
