...
function lpd_product_category_add_new_meta_field() {
	...
	<div class="form-field">
		...
		<div id="category_thumbnail1" style="float:left;margin-right:10px;"><img src="<?php echo THEME_ASSETS.'images/placeholder.png'; ?>" width="60px" height="60px" /></div>
		<div style="line-height:60px;">
			<input type="hidden" id="term_meta[custom_term_meta]" class="category_thumbnail1_id" name="term_meta[custom_term_meta]" />
		...
		<script type="text/javascript">
			 ...
			 if ( ! jQuery('.category_thumbnail1_id').val() )
			...
			jQuery(document).on( 'click', '.upload_image_button2', function( event ){
				...
				file_frame1.on( 'select', function() {
					...
					jQuery('.category_thumbnail1_id').val( attachment.id );
					jQuery('#category_thumbnail1 img').attr('src', attachment.url );
			...
			jQuery(document).on( 'click', '.remove_image_button2', function( event ){
				jQuery('#category_thumbnail1 img').attr('src', '<?php echo THEME_ASSETS.'images/placeholder.png'; ?>');
				jQuery('.category_thumbnail1_id').val('');
				jQuery('.remove_image_button2').hide();
				return false;
			});
...
function lpd_product_category_edit_meta_field($term) {
	...
	<tr class="form-field">
		...
		<td>
			<div id="category_thumbnail1" style="float:left;margin-right:10px;"><img src="<?php echo esc_url($image);?>" width="60px" height="60px" /></div>
			<div style="line-height:60px;">
				<input type="hidden" id="term_meta[custom_term_meta]" class="category_thumbnail1_id" name="term_meta[custom_term_meta]" value="<?php echo esc_attr( $term_meta['custom_term_meta'] ) ? esc_attr( $term_meta['custom_term_meta'] ) : ''; ?>" />
				<button type="submit" class="upload_image_button2 button"><?php _e( 'Upload/Add image', GETTEXT_DOMAIN ); ?></button>
				<button type="submit" class="remove_image_button2 button"><?php _e( 'Remove image', GETTEXT_DOMAIN ); ?></button>
			</div>
			<script type="text/javascript">
