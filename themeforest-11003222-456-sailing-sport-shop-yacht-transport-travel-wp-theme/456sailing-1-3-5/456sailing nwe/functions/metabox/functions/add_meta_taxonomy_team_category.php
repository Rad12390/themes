<?php
function lpd_team_category_add_new_meta_field() {
	wp_enqueue_media();
	?>
...
add_action( 'team_category_add_form_fields', 'lpd_team_category_add_new_meta_field', 10, 2 );

function lpd_team_category_edit_meta_field($term) {
	
 	wp_enqueue_media();

	$t_id = $term->term_id;
 
	$term_meta = get_option( "team_category_$t_id" );
...
add_action( 'team_category_edit_form_fields', 'lpd_team_category_edit_meta_field', 10, 2 );

function lpd_team_category_save_custom_meta( $term_id ) {
	if ( isset( $_POST['term_meta'] ) ) {
		...
		update_option( "team_category_$t_id", $term_meta );
	}
}  
add_action( 'edited_team_category', 'lpd_team_category_save_custom_meta', 10, 2 );  
add_action( 'create_team_category', 'lpd_team_category_save_custom_meta', 10, 2 );