...
// Add the Meta Box
function add_team_header_meta_box() {
    add_meta_box(
		'team_header', // $id
		__('Page Header', GETTEXT_DOMAIN), // $title 
		'show_team_header_meta_box', // $callback
		'team', // $page
		'normal', // $context
		'high'); // $priority
}
add_action('add_meta_boxes', 'add_team_header_meta_box');

// Field Array
$prefix = 'team_header_';
$team_header_meta_fields = array(
	array(
...
add_action('admin_head','add_team_header_scripts');
function add_team_header_scripts() {
	global $team_header_meta_fields, $post, $pagenow;
	if ( $pagenow == 'post-new.php' || $pagenow == 'post.php' ) {
	
	$output = '<script type="text/javascript">
				jQuery(function() {';
	
	foreach ($team_header_meta_fields as $field) { // loop through the fields looking for certain types
		// date
		if($field['type'] == 'date')
...
// The Callback
function show_team_header_meta_box() {
	global $team_header_meta_fields, $post;
	// Use nonce for verification
	echo '<input type="hidden" name="team_header_meta_box_nonce" value="'.wp_create_nonce(basename(__FILE__)).'" />';
	
	// Begin the field table and loop
	echo '<table class="form-table">';
	foreach ($team_header_meta_fields as $field) {
...
// Save the Data
function save_team_header_meta($post_id) {
    global $team_header_meta_fields;
	
	// verify nonce
	$nonce = '';
	if(isset($_POST['team_header_meta_box_nonce'])){ $nonce = $_POST['team_header_meta_box_nonce']; }
...
add_action('save_post', 'save_team_header_meta');
