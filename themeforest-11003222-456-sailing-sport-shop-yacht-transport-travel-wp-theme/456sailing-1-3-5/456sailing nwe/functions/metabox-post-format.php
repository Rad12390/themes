...
$metaboxes = array(
    'link_post' => array(
        'title' => esc_html__('Link Post', GETTEXT_DOMAIN),
        'applicableto' => 'post',
        'location' => 'normal',
        ...
        'fields' => array(
            'link_post_meta' => array(
                'title' => esc_html__('Url for link post: ', GETTEXT_DOMAIN),
                'type' => 'text',
                'description' => '',
                'size' => 60
            )
    ...
    'video_post' => array(
        'title' => esc_html__('Video Url (youtube or vimeo)', GETTEXT_DOMAIN),
        'applicableto' => 'post',
        'location' => 'normal',
        ...
        'fields' => array(
            'video_post_meta' => array(
                'title' => esc_html__('Enter video url: ', GETTEXT_DOMAIN),
                'type' => 'text',
                'description' => '',
                'size' => 60
            )
        ...
        foreach ( $fields as $id => $field ) {
            switch ( $field['type'] ) {
                ...
                case "text":
                    ...
                    $output .= '<label for="' . esc_attr($id) . '">' . $field['title'] . '</label><input id="' . esc_attr($id) . '" type="text" name="' . esc_attr($id) . '" value="' . esc_attr($custom[$id][0]) . '" size="' . esc_attr($field['size']) . '" />';
