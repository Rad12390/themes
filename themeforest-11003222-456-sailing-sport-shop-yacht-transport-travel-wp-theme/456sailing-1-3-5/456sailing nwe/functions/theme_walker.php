			...
			$classes = empty( $item->classes ) ? array() : (array) $item->classes;
				...
				if($this->isnavMenu == "standard") {
					...
					if($depth==0){
						...
						$item_output .= '<span data-hover="'.apply_filters( 'the_title', $item->title, $item->ID ).'">';
					...
					elseif(in_array("thumbnail-item", $classes)&&$depth==2){
						...
						if ( 'product' == get_post_type($item->object_id) ){
							...
							if ($product_id->is_on_sale()) :
								...
								$item_output .= apply_filters('woocommerce_sale_flash', '<span class="lpd-onsale">'.esc_html__('Sale!', GETTEXT_DOMAIN).'</span>', $item->object_id, $product_id);
							...
							if ( ! $product_id->is_in_stock() ) :
								$item_output .= '<span class="lpd-out-of-s'.$on_sale_products.'">'.esc_html__( 'Out of Stock', GETTEXT_DOMAIN ).'</span>';
							endif;
...
class lpd_admin_menus_walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( esc_html__( '%s (Invalid)', GETTEXT_DOMAIN), $item->title );
		}
		elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( esc_html__('%s (Pending)', GETTEXT_DOMAIN), $item->title );
		}
			...
			<dl class="menu-item-bar">
				<dt class="menu-item-handle">
					...
					<span class="item-controls">
						...
						?>"><?php esc_html_e('Edit Menu Item', GETTEXT_DOMAIN); ?></a>
					</span>
				</dt>
			</dl>
			...
			<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
				<?php if( 'custom' == $item->type ) : ?>
					<p class="field-url description description-wide">
						<label for="edit-menu-item-url-<?php echo $item_id; ?>">
							<?php esc_html_e( 'URL', GETTEXT_DOMAIN); ?><br />
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo $item_id; ?>">
						<?php esc_html_e( 'Navigation Label', GETTEXT_DOMAIN); ?><br />
						<input type="text" id="edit-menu-item-title-<?php echo $item_id; ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo $item_id; ?>">
						<?php esc_html_e( 'Title Attribute', GETTEXT_DOMAIN); ?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo $item_id; ?>">
						...
						<?php esc_html_e( 'Open link in a new window/tab', GETTEXT_DOMAIN); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo $item_id; ?>">
						<?php esc_html_e( 'CSS Classes (optional)', GETTEXT_DOMAIN); ?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo $item_id; ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo $item_id; ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo $item_id; ?>">
						<?php esc_html_e( 'Link Relationship (XFN)', GETTEXT_DOMAIN); ?><br />
						<input type="text" id="edit-menu-item-xfn-<?php echo $item_id; ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
					</label>
				</p>
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo $item_id; ?>">
						<?php esc_html_e( 'Description', GETTEXT_DOMAIN); ?><br />
						<textarea id="edit-menu-item-description-<?php echo $item_id; ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo $item_id; ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
						<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', GETTEXT_DOMAIN); ?></span>
					</label>
				</p>
				...
				<div class="menu-item-actions description-wide submitbox">
                   ...
                   <label for="menu-item-nav-type-<?php echo $item_id; ?>"><?php esc_html_e('Navigation Type', GETTEXT_DOMAIN); ?>:</label>
                   <input type="radio" id="menu-item-nav-type-<?php echo $item_id; ?>"  name="menu-item-nav-type-<?php echo $item_id; ?>" <?php  echo $value_standard; ?> value="standard" /> <?php esc_html_e('Standard', GETTEXT_DOMAIN); ?> &nbsp&nbsp
                   <input type="radio" id="menu-item-nav-type-<?php echo $item_id; ?>"  name="menu-item-nav-type-<?php echo $item_id; ?>" <?php  echo $value_mega; ?> value="mega" /> <?php esc_html_e('Mega', GETTEXT_DOMAIN); ?> &nbsp&nbsp
                   <input type="radio" id="menu-item-nav-type-<?php echo $item_id; ?>"  name="menu-item-nav-type-<?php echo $item_id; ?>" <?php  echo $value_full_width; ?> value="full_width" /> <?php esc_html_e('Full Width', GETTEXT_DOMAIN); ?> &nbsp&nbsp
				
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( esc_html__('Original: %s', GETTEXT_DOMAIN), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					); ?>"><?php esc_html_e('Remove', GETTEXT_DOMAIN); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo $item_id; ?>" href="<?php	echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
						?>#menu-item-settings-<?php echo $item_id; ?>"><?php esc_html_e('Cancel', GETTEXT_DOMAIN); ?></a>
