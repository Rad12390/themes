<?php function lpd_title() {?>
	...
	<?php if(is_home()){
		...
		if ( is_search() ) {
			printf( esc_html__( 'Search Results: &ldquo;%s&rdquo;', 'woocommerce' ), get_search_query() );
			if ( get_query_var( 'paged' ) )
			printf( esc_html__( '&nbsp;&ndash; Page %s', 'woocommerce' ), get_query_var( 'paged' ) );
		}else{
	    ...
	    if ( is_day() ) :
	        printf(esc_html(get_the_date('M j, Y')));
	    elseif ( is_month() ) :
	        printf(esc_html(get_the_date('F Y')));
	    elseif ( is_year() ) :
	        printf(esc_html(get_the_date('Y')));
	    elseif(is_category()) :
	    	single_cat_title();
	    elseif(is_tag()) :
	    	single_tag_title();
