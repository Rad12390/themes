...
function lpd_language_selector_flags(){
	...
	<?php if(!empty($languages)){
        ...
        if($l['active']) echo '<span class="current-lang">';
        	...
        	if($l['active']) echo '<span class="icon"></span><span class="language_code">'.esc_html($l['language_code']).'</span>';
...
<?php }
function lpd_language_selector_flags_panel(){
    $languages = icl_get_languages('skip_missing=0&orderby=code');
    if(!empty($languages)){
        foreach($languages as $l){
        echo '<span class="flag">';
            if(!$l['active']) echo '<a href="'.esc_url($l['url']).'">';
            echo '<img title="'.esc_attr($l['translated_name']).'" src="'.esc_attr($l['country_flag_url']).'" alt="'.esc_attr($l['language_code']).'" height="12"/>';
...
function lpd_post_gallery( $output, $attr) {
    ...
    foreach ( $attachments as $id => $attachment ) {
        $link = isset($attr['link']) && 'file' == $attr['link'] ? wp_get_attachment_link($id, 'theme-size-4x3', false, false) : wp_get_attachment_link($id, 'theme-size-4x3', true, false);
