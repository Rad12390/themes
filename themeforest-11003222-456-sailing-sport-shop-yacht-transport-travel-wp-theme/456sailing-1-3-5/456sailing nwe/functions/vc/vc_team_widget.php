...
function vc_team_widget_func( $atts, $content = null ) { // New function parameter $content is added!
   ...
   if($title_bg_color){
	   $title_styles = ' style="background-color:'.esc_attr($title_bg_color).'"';
   }
		...
		<?php if ( !$carousel ) { ?><div class="col-md-<?php echo esc_attr($columns) ?>"><?php }?>
			<div class="team-widget-item">
		       ...
		       <div class="team-widget-content">
					<div class="tw-title-wrap"<?php echo $title_styles;?>>
						<h3 class="tw-title">
					...
					<div class="tw_content">
						...
						<div class="tw-icon-wrap">
							<div class="tw-icon"<?php echo $icon_styles;?>>
								<img class="tw-icon-img" alt="" src="<?php echo esc_url($icon[0]);?>">
							</div>
						</div>
						<?php }?>
					...
					<?php if($social_media){?>		                        
						...
						<div class="team-widget-social-details meta_colored_icons picons_social lpd-animated-link">
							<ul>
...
vc_map(array(
   ...
   "params" => array(
		...
		array(
			"type" => "colorpicker",
			"class" => "",
			"heading" => __("Title Background Color", GETTEXT_DOMAIN),
