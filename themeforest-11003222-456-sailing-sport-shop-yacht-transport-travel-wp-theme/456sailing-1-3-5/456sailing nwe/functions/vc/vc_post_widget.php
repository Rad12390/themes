...
function vc_widget_func( $atts, $content = null ) { // New function parameter $content is added!
   ...
   if($title_font_size){$title_font_size = 'font-size:'.esc_attr($title_font_size).'px;';}
   ...
   if($title_uppercase){$title_uppercase = 'text-transform:uppercase;';}
   
   $styles_h4 = '';
	
   if($title_font_size||$title_font_weight||$title_letter_spacing){
	   $styles_h4 = ' style="'.$title_font_weight.''.$title_font_size.''.$title_letter_spacing.''.$title_uppercase.'"';
   }
   
   $meta_styles = '';
   
   if($meta_font_weight){
	   $meta_styles = ' style="font-weight:'.esc_attr($meta_font_weight).'"';
   }
   
   $icon_styles = '';
   
   if($icon_size){
	   $icon_styles = ' style="font-size:'.esc_attr($icon_size).'px"';
   }
	...
	<?php if ( !$carousel ) { ?><div class="col-md-<?php echo esc_attr($columns) ?>"><?php }?>
	    ...
	    <?php }?>	
			<div class="content<?php if ( $thumbnails ) { ?> no-thumbnail<?php }?>">
				<div class="widget-meta lpd-animated-link"<?php echo $meta_styles;?>>
					<table>
						<tbody>
							<tr>
				...
				<?php if ( $link ) { ?>
				<h4 class="title lpd-animated-link<?php if($title_font_style){?> pw-font-style<?php } ?>"<?php echo $styles_h4;?>><a href="<?php echo esc_url($link); ?>"><?php the_title(); ?></a></h4>
				<?php }else{?>
				<h4 class="title lpd-animated-link<?php if($title_font_style){?> pw-font-style<?php } ?>"<?php echo $styles_h4;?>><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h4>
				<?php }?>
				<?php if ( !$caption ) { ?>
				...
				<div class="pw-icon-wrap">
					<div class="pw-icon"<?php echo $icon_styles;?>>
						<img class="pw-icon-img" alt="" src="<?php echo esc_url($icon[0]);?>">
					</div>
				</div>
				<?php }?>
			</div>
...
vc_map(array(
   ...
   "params" => array(
		...
		array(
			"type" => 'checkbox',
			"heading" => __("Uppercase Title", GETTEXT_DOMAIN),
			"param_name" => "title_uppercase",
			"description" => __("Check, if you want to enable uppercase text for title.", GETTEXT_DOMAIN),
			"value" => Array(__("Enable", GETTEXT_DOMAIN) => 'enable')
		),
		array(
			"type" => "dropdown",
			"heading" => __("Meta Font Weight", GETTEXT_DOMAIN),
			...
			"description" => __("Select meta font weight.", GETTEXT_DOMAIN)
		),
	    array(
	      "type" => "attach_image",
