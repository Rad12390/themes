...
function vc_new_button_func( $atts, $content = null ) {
   extract( shortcode_atts( array(
    ...
    return '<a class="btn btn-vc btn-'. esc_attr($type) .' btn-'. esc_attr($size) .'" href="'. esc_url($link) .'" style="'. esc_attr($display) .''. esc_attr($align) .''. esc_attr($styles) .'">'. esc_html($text) .'</a>';
}
add_shortcode( 'vc_new_button', 'vc_new_button_func' );

vc_map(array(
   "name" => __("Buttons", GETTEXT_DOMAIN),
   "base" => "vc_new_button",
   "class" => "",
   "icon" => "icon-wpb-lpd",
