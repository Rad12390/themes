...
function vc_lpd_header_func( $atts, $content = null ) {
   ...
   $content = wpb_js_remove_wpautop($content); // fix unclosed/unwanted paragraph tags in $content
	...
	if($content_color||$letter_spacing){
		...
		if($content_alignment){$content_styles .= 'text-align:'.esc_attr($content_alignment).';';}
		$content_styles .= '"';
	}
...
__('Right', GETTEXT_DOMAIN) => "right",
		...
		array(
			"type" => "dropdown",
			"heading" => __("Content Alignment", GETTEXT_DOMAIN),
...
__('Right', GETTEXT_DOMAIN) => "right",
			...
			"description" => __("Select the alignment for content.", GETTEXT_DOMAIN)
		),
		array(
			"type" => "colorpicker",
			"holder" => "div",
