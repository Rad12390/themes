...
global $wp_query;
	...
	if(is_singular('essential_grid')){
		$essential_grid_padding_top = get_post_meta($posts_page->ID, 'essential_grid_padding_top', true);
		$essential_grid_padding_bottom = get_post_meta($posts_page->ID, 'essential_grid_padding_bottom', true);
		if($essential_grid_padding_top||$essential_grid_padding_bottom){
			$out = '';
			$out .= ' style="';
			if($essential_grid_padding_top){
				$out .= 'padding-top:'.esc_attr($essential_grid_padding_top).'px;';
			}
			if($essential_grid_padding_bottom){
				$out .= 'padding-bottom:'.esc_attr($essential_grid_padding_bottom).'px;';
			} 
			$out .= '"';
			return $out;
		}
