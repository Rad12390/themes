...
$portfolio_thumbnails_a_type = ot_get_option('portfolio_thumbnails_a_type');
$blog_front_a_type = ot_get_option('blog_front_a_type');
$blog_post_a_type = ot_get_option('blog_post_a_type');


if($sidebar_a_type||$footer_a_type||$footer_top_a_type||$footer_bottom_a_type||$footer_meta_a_type||$page_title_a_type||$shop_thumb_a_type||$product_page_a_type||$product_image_a_type||$product_bottom_a_type||$team_post_a_type||$portfolio_post_a_type||$portfolio_thumbnails_a_type||$blog_front_a_type||$blog_post_a_type){
...
if(!$disable_sticky){
	add_action( 'wp_enqueue_scripts', 'menu3d1' );
    add_action( 'wp_enqueue_scripts', 'lpd_sticky_menu' );
    
}

if (is_plugin_active('sitepress-multilingual-cms/sitepress.php')||is_plugin_active('woocommerce/woocommerce.php')){
	if ($s_cart=='sl_panel_right'||$s_cart=='sl_panel_left'||$wpml_switcher=='sl_panel_top') {
